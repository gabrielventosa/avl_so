...
def ProcessCMD(cmd,number):
    SER.send('Starting remote command operations\r\n')
    ...
    if(cmd.find('SALDO') != -1):
        SER.send('Requesting SALDO from TAE\r\n')
        res = SMS.sendSMS('333','SALDO')
        if(res == -1):
            return
        msg = -1
        while (msg == -1):
            SER.send('...Waiting for Message\r\n')
            msg = SMS.check4SMS()
        data = SMS.ReadMessage(msg)
        SMS.DelMessage(msg)
        if(data['phn'] == 'TAE'):
            SER.send('Message received from TAE\r\n')
    ...
    if(cmd.find('GPSPOS') !=-1):
        ...
        if (data == -1):
            ...
            SER.send('Sending GPS info via SMS\r\n')
            SER.send('···:')
            SER.send(pos)
            SER.send('\r\n')
